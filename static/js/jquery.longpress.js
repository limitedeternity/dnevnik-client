    ...
    $.fn.longpress = function(longCallback, shortCallback, duration) {
        ...
        return this.each(function() {
            ...
            function mousedown_callback(e) {
                mouse_down_x = e.touches ? e.touches[0].clientX : e.clientX;
                mouse_down_y = e.touches ? e.touches[0].clientY : e.clientY;
            ...
            function mouseup_callback(e) {
                mouse_up_x = e.touches ? e.touches[0].clientX : e.clientX;
                mouse_up_y = e.touches ? e.touches[0].clientY : e.clientY;

                if(Math.abs(mouse_up_x - mouse_down_x) > 100 || Math.abs(mouse_up_y - mouse_down_y) > 100) {return;}

                if(new Date().getTime() - mouse_down_time >= duration) {return;}
                ...
                if (typeof shortCallback === "function") {
                  ...
                  // (mouseup, touchend)
                  if(new Date().getTime() - short_call_time > 50) {
                    shortCallback.call($(this), e);
                  }
                  short_call_time = new Date().getTime();
